...
(function ($) {
    ...
    get: function (total, num) {
      ...
      $.post(ajaxurl, data, function (r) {
        ...
        if ('object' == typeof r && r.responses[0]) {
          ...
          if (commentsBox.st > commentsBox.total) $('#show-comments').hide();
          else $('#show-comments').show().children('a').text(__('Show more comments'));
   ...
   */
  $(document)
    .on('heartbeat-send.refresh-lock', function (e, data) {
      var lock = $('#active_post_lock').val(),
        post_id = $('#post_ID').val(),
        send = {};

      if (!post_id || !$('#post-lock-dialog').length) return;

      send.post_id = post_id;

      if (lock) send.lock = lock;
...
jQuery(function ($) {
  ...
  $submitButtons = $submitpost
    .find(':submit, a.submitdelete, #post-preview')
    .on('click.edit-post', function (event) {
      ...
      $('form#post')
        .off('submit.edit-post')
        .on('submit.edit-post', function (event) {
  ...
  $document
    .on('autosave-disable-buttons.edit-post', function () {
      $submitButtons.addClass('disabled');
    })
   ...
   */
  $(window)
    .on('beforeunload.edit-post', function (event) {
  ...
  $('.categorydiv').each(function () {
    ...
    catAddAfter = function (r, s) {
      var sup,
        drop = $('#new' + taxonomy + '_parent');

      $('#' + taxonomy + '-add-submit').prop('disabled', false);
      if (
        'undefined' != s.parsed.responses[0] &&
        (sup = s.parsed.responses[0].supplemental.newcat_parent)
      ) {
        drop.before(sup);
        drop.remove();
      }
    ...
    $('#' + taxonomy + 'checklist, #' + taxonomy + 'checklist-pop').on(
      ...
      function () {
        ...
        if (id && t.parents('#taxonomy-' + taxonomy).length) {
          $(
            'input#in-' + taxonomy + '-' + id + ', input[id^="in-' + taxonomy + '-' + id + '-"]'
          ).prop('checked', c);
  ...
  if ($('#submitdiv').length) {
     ...
     */
    updateText = function () {
      if (!$timestampdiv.length) return true;
      ...
      if ($postVisibilitySelect.find('input:radio:checked').val() == 'private') {
        ...
        if (postStatus.is(':hidden')) $('#misc-publishing-actions .edit-post-status').show();
      }
    ...
    $postVisibilitySelect.find('.save-post-visibility').on('click', function (event) {
      // Crazyhorse branch - multiple OK cancels.
      var visibilityLabel = '',
        selectedVisibility = $postVisibilitySelect.find('input:radio:checked').val();
  ...
  function editPermalink() {
    ...
    $el
      .html(
        '<label for="new-post-slug" class="screen-reader-text">' +
          slug_label +
          '</label>' +
  ...
  (function () {
    ...
    $handle
      .on('mousedown.wp-editor-resize', function (event) {
        ...
        $document
          .on('mousemove.wp-editor-resize', dragging)
