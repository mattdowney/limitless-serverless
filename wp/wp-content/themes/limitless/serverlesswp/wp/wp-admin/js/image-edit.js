  ...
  var imageEdit = (window.imageEdit = {
    ...
    toggleCropTool: function (postid, nonce, cropButton) {
      ...
      if ('false' === state) {
        ...
        var startX = $('#imgedit-start-x-' + postid).val()
          ? $('#imgedit-start-x-' + postid).val()
          : 0;
        var startY = $('#imgedit-start-y-' + postid).val()
          ? $('#imgedit-start-y-' + postid).val()
          : 0;
        var width = $('#imgedit-sel-width-' + postid).val()
          ? $('#imgedit-sel-width-' + postid).val()
          : img.innerWidth();
        var height = $('#imgedit-sel-height-' + postid).val()
    ...
    togglePopup: function (el) {
      ...
      var $target = $('#' + $targetEl);
      $el.attr('aria-expanded', 'false' === $el.attr('aria-expanded') ? 'true' : 'false');
      // Open menu and set z-index to appear above image crop area if it is enabled.
      $target.toggleClass('imgedit-popup-menu-open').slideToggle('fast').css({ 'z-index': 200000 });
    ...
    closePopup: function (el) {
      ...
      $target.attr('aria-expanded', 'false').trigger('focus');
      $parent.toggleClass('imgedit-popup-menu-open').slideToggle('fast');
    ...
    toggleControls: function (el) {
      ...
      $el.attr('aria-expanded', 'false' === $el.attr('aria-expanded') ? 'true' : 'false');
      $target.parent('.imgedit-group').toggleClass('imgedit-panel-active');
    ...
    scaleChanged: function (postid, x, el) {
      var w = $('#imgedit-scale-width-' + postid),
        h = $('#imgedit-scale-height-' + postid),
        warn = $('#imgedit-scale-warn-' + postid),
        w1 = '',
        h1 = '',
    ...
    filterHistory: function (postid, setSize) {
      ...
      var history = $('#imgedit-history-' + postid).val(),
        pop,
        n,
        o,
        i,
      ...
      img = $('<img id="image-preview-' + postid + '" alt="" />')
        ...
        .on('error', function () {
          ...
          $('#imgedit-crop-' + postid)
            .empty()
            .append(
              '<div class="notice notice-error" tabindex="-1" role="alert"><p>' +
                errorMessage +
                '</p></div>'
            );
    ...
    save: function (postid, nonce) {
      ...
      $.post(ajaxurl, data, function (response) {
        ...
        if (response.data.msg) {
          $('#imgedit-response-' + postid).html(
            '<div class="notice notice-success" tabindex="-1" role="alert"><p>' +
              response.data.msg +
              '</p></div>'
          );
    ...
    undo: function (postid, nonce) {
      var t = this,
        button = $('#image-undo-' + postid),
        elem = $('#imgedit-undone-' + postid),
    ...
    setNumSelection: function (postid, el) {
      var sel,
        elX = $('#imgedit-sel-width-' + postid),
        elY = $('#imgedit-sel-height-' + postid),
        elX1 = $('#imgedit-start-x-' + postid),
        elY1 = $('#imgedit-start-y-' + postid),
        xS = this.intval(elX1.val()),
        yS = this.intval(elY1.val()),
        x = this.intval(elX.val()),
        y = this.intval(elY.val()),
        img = $('#image-preview-' + postid),
        imgh = img.height(),
        imgw = img.width(),
        sizer = this.hold.sizer,
        x1,
