...
(function ($, wp, settings) {
  ...
  wp.updates.refreshCount = function () {
    ...
    $dashboardNavMenuUpdateCount.each(function (index, element) {
      element.className = element.className.replace(
        /count-\d+/,
        'count-' + settings.totals.counts.total
      );
    });
    ...
    $pluginsNavMenuUpdateCount.each(function (index, element) {
      element.className = element.className.replace(
        /count-\d+/,
        'count-' + settings.totals.counts.plugins
      );
    });
    if (settings.totals.counts.total > 0) {
    ...
    $appearanceNavMenuUpdateCount.each(function (index, element) {
      element.className = element.className.replace(
        /count-\d+/,
        'count-' + settings.totals.counts.themes
      );
    });
    if (settings.totals.counts.total > 0) {
  ...
  wp.updates.setCardButtonStatus = function (data) {
    ...
    if (
      false !== $.support.postMessage &&
      null !== target &&
      -1 === window.parent.location.pathname.indexOf('index.php')
    ) {
  ...
  wp.updates.updatePlugin = function (args) {
    ...
    if ('plugins' === pagenow || 'plugins-network' === pagenow) {
      ...
      $message = $updateRow
        ...
        .addClass('updating-message notice-warning')
        .find('p');
      message = sprintf(
    ...
    $message.attr('aria-label', message).text(buttonText);
  ...
  wp.updates.updatePluginSuccess = function (response) {
    ...
    if ('plugins' === pagenow || 'plugins-network' === pagenow) {
      ...
      newText = $pluginRow
        .find('.plugin-version-author-uri')
        .html()
        .replace(response.oldVersion, response.newVersion);
      $pluginRow.find('.plugin-version-author-uri').html(newText);

      // Clear the "time to next auto-update" text.
      $pluginRow.find('.auto-update-time').empty();
    } else if ('plugin-install' === pagenow || 'plugin-install-network' === pagenow) {
      $updateMessage = $('.plugin-card-' + response.slug + ', #plugin-information-footer')
        .find('.update-now')
    ...
    $updateMessage.attr('aria-label', ariaLabel).text(buttonText);
  ...
  wp.updates.updatePluginError = function (response) {
    ...
    if ('plugins' === pagenow || 'plugins-network' === pagenow) {
      ...
      $card = $('.plugin-card-' + response.slug + ', #plugin-information-footer').append(
      ...
      $card.find('.update-now').text(buttonText).removeClass('updating-message');
      ...
      $card.on('click', '.notice.is-dismissible .notice-dismiss', function () {
        ...
        setTimeout(function () {
          ...
          $card.find('.update-now').attr('aria-label', false).text(__('Update Now'));
        }, 200);
      });
    }
  ...
  wp.updates.installPlugin = function (args) {
    ...
    $message.addClass('updating-message').attr('aria-label', ariaLabel).text(buttonText);
  ...
  wp.updates.installPluginSuccess = function (response) {
    var $message = $('.plugin-card-' + response.slug + ', #plugin-information-footer').find(
        '.install-now'
      ),
  ...
  wp.updates.installPluginError = function (response) {
    ...
    $card
      ...
      .append(
        '<div class="notice notice-error notice-alt is-dismissible" role="alert"><p>' +
          errorMessage +
          '</p></div>'
      );
  ...
  wp.updates.activatePlugin = function (args) {
    var $message = $('.plugin-card-' + args.slug + ', #plugin-information-footer').find(
      '.activate-now, .activating-message'
    );
  ...
  wp.updates.deletePlugin = function (args) {
    ...
    if ($link.html() !== __('Deleting...')) {
      $link.data('originaltext', $link.html()).text(__('Deleting...'));
    }
      ...
      .fadeOut(350, function () {
        ...
        if (-1 !== _.indexOf(plugins.recently_activated, response.plugin)) {
          ...
          if (plugins.recently_activated.length) {
            $views
              .find('.recently_activated .count')
              .text('(' + plugins.recently_activated.length + ')');
          } else {
        ...
        if (-1 !== _.indexOf(plugins['auto-update-enabled'], response.plugin)) {
          plugins['auto-update-enabled'] = _.without(
            plugins['auto-update-enabled'],
            response.plugin
          );
          if (plugins['auto-update-enabled'].length) {
            $views
              .find('.auto-update-enabled .count')
              .text('(' + plugins['auto-update-enabled'].length + ')');
          } else {
        ...
        if (-1 !== _.indexOf(plugins['auto-update-disabled'], response.plugin)) {
          plugins['auto-update-disabled'] = _.without(
            plugins['auto-update-disabled'],
            response.plugin
          );
          if (plugins['auto-update-disabled'].length) {
            $views
              .find('.auto-update-disabled .count')
              .text('(' + plugins['auto-update-disabled'].length + ')');
          } else {
        ...
        if (plugins.all.length) {
          ...
          if (!$form.find('tr.no-items').length) {
            $form
              .find('#the-list')
              .append(
                '<tr class="no-items"><td class="colspanchange" colspan="' +
                  columnCount +
                  '">' +
  ...
  wp.updates.updateTheme = function (args) {
    ...
    if ('themes-network' === pagenow) {
      ...
      $notice = $notice.add(
        $('#customize-control-installed_theme_' + args.slug).find('.update-message')
      );
  ...
  wp.updates.updateThemeSuccess = function (response) {
    ...
    if ('customize' === pagenow) {
      ...
      $notice = $('.theme-info .notice').add(
        wp.customize
          .control('installed_theme_' + response.slug)
  ...
  wp.updates.installTheme = function (args) {
    ...
    // Remove previous error messages, if any.
    $('.install-theme-info, [data-slug="' + args.slug + '"]')
      .removeClass('theme-install-failed')
  ...
  wp.updates.deleteTheme = function (args) {
    ...
    if ($button && $button.html() !== __('Deleting...')) {
      $button.data('originaltext', $button.html()).text(__('Deleting...'));
    }
      ...
      $themeRows.css({ backgroundColor: '#faafaa' }).fadeOut(350, function () {
        ...
        if (-1 !== _.indexOf(themes['auto-update-enabled'], response.slug)) {
          ...
          if (themes['auto-update-enabled'].length) {
            $views
              .find('.auto-update-enabled .count')
              .text('(' + themes['auto-update-enabled'].length + ')');
          } else {
            $views.find('.auto-update-enabled').remove();
          }
        }
        ...
        if (-1 !== _.indexOf(themes['auto-update-disabled'], response.slug)) {
          ...
          if (themes['auto-update-disabled'].length) {
            $views
              .find('.auto-update-disabled .count')
              .text('(' + themes['auto-update-disabled'].length + ')');
          } else {
            $views.find('.auto-update-disabled').remove();
          }
        }
  ...
  wp.updates.showErrorInCredentialsForm = function (message) {
    ...
    $filesystemForm
      .find('#request-filesystem-credentials-title')
      .after(
        '<div class="notice notice-alt notice-error" role="alert"><p>' + message + '</p></div>'
      );
  };
  ...
  $(function () {
    ...
    $filesystemModal.on('submit', 'form', function (event) {
      ...
      wp.updates.filesystemCredentials.ftp.connectionType = $(
    ...
    $filesystemModal.on(
      'click',
      '[data-js-action="close"], .notification-dialog-background',
      wp.updates.requestForCredentialsModalCancel
    );
    ...
    $document.on('credential-modal-cancel', function (event, job) {
      ...
      if ($message && $message.hasClass('updating-message')) {
        ...
        $message.removeClass('updating-message').html(originalText);
    ...
    $pluginFilter.on('click', '.install-now', function (event) {
      ...
      if (wp.updates.shouldRequestFilesystemCredentials && !wp.updates.ajaxLocked) {
        ...
        $document.on('credential-modal-cancel', function () {
          ...
          $message.removeClass('updating-message').text(_x('Install Now', 'plugin'));
    ...
    $bulkActionForm.on(
      'click',
      '[type="submit"]:not([name="clear-recent-list"])',
      function (event) {
        ...
        itemsSelected.each(function (index, element) {
          ...
          if (
            'update-selected' === bulkAction &&
            (!$itemRow.hasClass('update') || $itemRow.find('notice-error').length)
          ) {
            // Un-check the box.
        ...
        $document.on(
          'wp-plugin-update-success wp-plugin-update-error wp-theme-update-success wp-theme-update-error',
          function (event, response) {
            ...
            if ('wp-' + response.update + '-update-success' === event.type) {
              ...
              itemName = response.pluginName
                ? response.pluginName
                : $itemRow.find('.column-primary strong').text();
              ...
              if ('plugin' === response.update) {
                successMessage = sprintf(
                  ...
                  _n(
                    ...
                    '%s plugins successfully updated.',
                    success
                  ),
                  success
                );
     ...
     * @since 4.6.0
     */
    $pluginInstallSearch.on(
      'keyup input',
      _.debounce(function (event, eventtype) {
        var $searchTab = $('.plugin-install-search'),
          data,
          searchLocation,
        ...
        searchLocation =
          location.href.split('?')[0] + '?' + $.param(_.omit(data, ['_ajax_nonce', 'pagenow']));
        ...
        if (!$searchTab.length) {
          $searchTab = $('<li class="plugin-install-search" />').append(
            $('<a />', {
              class: 'current',
              href: searchLocation,
        ...
        wp.updates.searchRequest = wp.ajax
          .post('search-install-plugins', data)
      ...
      _.debounce(function (event) {
        ...
        if (window.history && window.history.replaceState) {
          window.history.replaceState(
            null,
            '',
            location.href.split('?')[0] + '?s=' + data.s + '&plugin_status=' + data.plugin_status
          );
        }
    ...
    $('#plugin_update_from_iframe').on('click', function (event) {
      ...
      if (
        false === $.support.postMessage ||
        null === target ||
        -1 !== window.parent.location.pathname.indexOf('update-core.php')
      ) {
        return;
      }
    ...
    $document.on(
      'keydown',
      '.column-auto-updates .toggle-auto-update, .theme-overlay .toggle-auto-update',
      function (event) {
        if (32 === event.which) {
          event.preventDefault();
        }
      ...
      function (event) {
        ...
        $.post(window.ajaxurl, data)
          .done(function (response) {
            ...
            $document.trigger('wp-auto-update-setting-changed', {
              state: action,
              type: type,
              asset: asset,
            });
