...
(function ($) {
  ...
  $.fn.allOffsets = function () {
    var offset = this.offset() || { top: 0, left: 0 },
      win = $(window);
  ...
  revisions.model.Slider = Backbone.Model.extend({
    ...
    getSliderValue: function (a, b) {
      return isRtl
        ? this.revisions.length - this.revisions.indexOf(this.get(a)) - 1
        : this.revisions.indexOf(this.get(b));
    },

    updateSliderSettings: function () {
      if (this.get('compareTwoMode')) {
        this.set({
          values: [this.getSliderValue('to', 'from'), this.getSliderValue('from', 'to')],
          value: null,
    ...
    _loadAll: function (allRevisionIds, centerId, num) {
      ...
      if (_.size(diffs) > 0) {
        this.load(diffs)
          ...
          .fail(function () {
            if (1 === num) {
              // Already tried 1. This just isn't working. Give up.
              deferred.reject();
            } else {
  ...
  revisions.model.FrameState = Backbone.Model.extend({
    ...
    changeMode: function (model, value) {
      ...
      if (!value && 0 !== toIndex) {
        // '! value' means switching to single-handled mode.
        this.set({
    ...
    receiveDiff: function (diff) {
      ...
      if (_.isUndefined(diff) || _.isUndefined(diff.id)) {
        ...
        // Make sure the current diff didn't change.
      ...
      this.window.on('scroll.wp.revisions', { controls: this }, function (e) {
        ...
        if (scrolled >= controls.top) {
          if (!frame.$el.hasClass('pinned')) {
            ...
            controls.window.on(
              'resize.wp.revisions.pinning click.wp.revisions.pinning',
              { controls: controls },
              function (e) {
  ...
  revisions.view.Tickmarks = wp.Backbone.View.extend({
    ...
    reportTickPosition: function (model, revision) {
      var offset,
        ...
        parentOffset,
        tick,
        index = this.model.revisions.indexOf(revision);
    ...
    ready: function () {
      ...
      _(tickCount).times(function (index) {
        this.$el.append(
          '<div style="' + this.direction + ': ' + 100 * tickWidth * index + '%"></div>'
        );
      }, this);
    },
  });
    ...
    toggleVisibility: function () {
      if (this.visible()) {
        this.$el
          .stop()
          .show()
          .fadeTo(100 - this.el.style.opacity * 100, 1);
      } else {
  ...
  revisions.view.Slider = wp.Backbone.View.extend({
    ...
    applySliderSettings: function () {
      ...
      if (this.model.get('compareTwoMode')) {
        ...
        handles.first().toggleClass('to-handle', !!isRtl).toggleClass('from-handle', !isRtl);
        handles.last().toggleClass('from-handle', !!isRtl).toggleClass('to-handle', !isRtl);
