...
jQuery(function ($) {
  ...
  $('#the-list').on('click', '.delete-tag', function () {
    ...
    if ('undefined' != showNotice) r = showNotice.warn();

    if (r) {
      data = t
        .attr('href')
        .replace(/[^?]*\?/, '')
        .replace(/action=delete/, 'action=delete-tag');
  ...
  $('#submit').on('click', function () {
    ...
    $.post(ajaxurl, $('#addtag').serialize(), function (r) {
      ...
      if (form.find('select#parent')) {
        ...
        indent = '';
        for (i = 0; i < res.responses[1].position; i++) indent += '&nbsp;&nbsp;&nbsp;';

        form
          .find('select#parent option:selected')
          .after('<option value="' + term.term_id + '">' + indent + term.name + '</option>');
      }

      $(
        'input:not([type="checkbox"]):not([type="radio"]):not([type="button"]):not([type="submit"]):not([type="reset"]):visible, textarea:visible',
        form
      ).val('');
    });
