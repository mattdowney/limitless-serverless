...
(function (api, wp, $) {
  ...
  api.Menus.AvailableItemCollection = Backbone.Collection.extend(
    /** @lends wp.customize.Menus.AvailableItemCollection.prototype */ {
  ...
  api.Menus.availableMenuItems = new api.Menus.AvailableItemCollection(
    api.Menus.data.availableMenuItems
  );
  ...
  api.Menus.AvailableMenuItemsPanelView = wp.Backbone.View.extend(
    /** @lends wp.customize.Menus.AvailableMenuItemsPanelView.prototype */ {
      ...
      initialize: function () {
        ...
        this.$el.on(
          'input',
          '#custom-menu-item-name.invalid, #custom-menu-item-url.invalid',
          function () {
        ...
        this.sectionContent.on('scroll', function () {
          var totalHeight = self.$el
              .find('.accordion-section.open .available-menu-items-list')
              .prop('scrollHeight'),
          ...
          if (!self.loading && $(this).scrollTop() > (3 / 4) * totalHeight - visibleHeight) {
            ...
            if ('search' === type) {
              ...
              self.loadItems([{ type: type, object: object }]);
            }
          }
        });
      ...
      loadItems: function (itemTypes, deprecated) {
        ...
        _.each(_itemTypes, function (itemType) {
          var container,
            name = itemType.type + ':' + itemType.object;
          ...
          _.each(data.items, function (typeItems, name) {
            if (0 === typeItems.length) {
              if (0 === self.pages[name]) {
                availableMenuItemContainers[name]
              ...
              'post_type:page' === name &&
              !availableMenuItemContainers[name].hasClass('open')
            ) {
              availableMenuItemContainers[name]
                .find('.accordion-section-title > button')
                .trigger('click');
      ...
      itemSectionHeight: function () {
        ...
        sections = this.$el.find(
          '.accordion-section:not( #available-menu-items-search ) .accordion-section-content'
        );
        lists = this.$el.find(
          '.accordion-section:not( #available-menu-items-search ) .available-menu-items-list:not(":only-child")'
        );
      ...
      submitLink: function () {
        ...
        if (!urlRegex.test(url) || '' === itemName.val()) {
          ...
          if ('' === itemName.val()) {
            ...
            errorText =
              '' === errorText ? nameErrorMessage.text() : errorText + nameErrorMessage.text();
            // Announce error message via screen reader
            wp.a11y.speak(errorText, 'assertive');
          }
          return;
  ...
  api.Menus.MenusPanel = api.Panel.extend(
    /** @lends wp.customize.Menus.MenusPanel.prototype */ {
      ...
      ready: function () {
        ...
        api.section('menu_locations', function (section) {
          section.headContainer.prepend(wp.template('nav-menu-locations-header')(api.Menus.data));
        });
      },
  ...
  api.Menus.MenuSection = api.Section.extend(
    /** @lends wp.customize.Menus.MenuSection.prototype */ {
      /**
       * Initialize.
       *
       * @since 4.3.0
      ...
      ready: function () {
        ...
        api.bind('pane-contents-reflowed', function () {
          ...
          section.container
            .find('.menu-item .menu-item-reorder-nav button')
            .attr({ tabindex: '0', 'aria-hidden': 'false' });
          section.container
            .find('.menu-item.move-up-disabled .menus-move-up')
            .attr({ tabindex: '-1', 'aria-hidden': 'true' });
          section.container
            .find('.menu-item.move-down-disabled .menus-move-down')
            .attr({ tabindex: '-1', 'aria-hidden': 'true' });
          section.container
            .find('.menu-item.move-left-disabled .menus-move-left')
            .attr({ tabindex: '-1', 'aria-hidden': 'true' });
          section.container
            .find('.menu-item.move-right-disabled .menus-move-right')
            .attr({ tabindex: '-1', 'aria-hidden': 'true' });
        });
        ...
        fieldActiveToggles = api
          .panel('nav_menus')
          .contentContainer.find('.metabox-prefs:first')
          .find('.hide-column-tog');
      ...
      highlightNewItemButton: function () {
        api.utils.highlightButton(this.contentContainer.find('.add-new-menu-item'), {
          delay: 2000,
        });
      },
    }
  ...
  api.Menus.createNavMenu = function createNavMenu(name) {
    ...
    setting.set(
      $.extend({}, api.Menus.data.defaultSettingValues.nav_menu, {
        name: name || '',
      })
    );
    ...
    return api.section.add(
      new api.Menus.MenuSection(customizeId, {
        panel: 'nav_menus',
  ...
  api.Menus.NewMenuSection = api.Section.extend(
    /** @lends wp.customize.Menus.NewMenuSection.prototype */ {
      /**
      ...
      attachEvents: function () {
        ...
        section.headContainer
          .find('.accordion-section-title')
          .replaceWith(wp.template('nav-menu-create-menu-section-title'));
      ...
      submit: function () {
        ...
        contentContainer.find('.assigned-menu-location input[type=checkbox]').each(function () {
          ...
          if (checkbox.prop('checked')) {
            navMenuLocationSetting = api(
              'nav_menu_locations[' + checkbox.data('location-id') + ']'
            );
  ...
  api.Menus.MenuLocationControl = api.Control.extend(
    /** @lends wp.customize.Menus.MenuLocationControl.prototype */ {
      initialize: function (id, options) {
        var control = this,
      ...
      ready: function () {
        ...
        api.bind('change', function (setting) {
          ...
          if (false === setting()) {
            ...
            control.container
              .find('option[value=' + menuId + ']')
              .text(displayNavMenuName(setting().name));
          }
        });
      },
    }
  ...
  api.Menus.MenuItemControl = api.Control.extend(
    /** @lends wp.customize.Menus.MenuItemControl.prototype */ {
      /**
      ...
      initAccessibility: function () {
        ...
        menu.on(
          'mouseenter.refreshAccessibility focus.refreshAccessibility touchstart.refreshAccessibility',
          '.menu-item',
          function () {
      ...
      refreshAdvancedAccessibilityOfItem: function (itemToRefresh) {
        ...
        if (isPrimaryMenuItem) {
          ((primaryItems = $('.menu-item-depth-0')),
            ...
            (title = menus.menuFocus
              .replace('%1$s', itemName)
              .replace('%2$s', menuItemType)
              .replace('%3$d', itemPosition)
              .replace('%4$d', totalMenuItems)));
        } else {
          ...
          if (depth < 2) {
            title = menus.subMenuFocus
              .replace('%1$s', itemName)
              .replace('%2$s', menuItemType)
              .replace('%3$d', itemPosition)
              .replace('%4$d', totalSubItems)
              .replace('%5$s', parentItemName);
          } else {
            title = menus.subMenuMoreDepthFocus
              .replace('%1$s', itemName)
              .replace('%2$s', menuItemType)
              .replace('%3$d', itemPosition)
              .replace('%4$d', totalSubItems)
      ...
      _setupReorderUI: function () {
        ...
        $reorderNav
          .find('.menus-move-up, .menus-move-down, .menus-move-left, .menus-move-right')
          .on('click', function () {
            var moveBtn = $(this);
      ...
      _setupUpdateUI: function () {
        ...
        control.elements.attr_title = new api.Element(
          control.container.find('.edit-menu-item-attr-title')
        );
        control.elements.target = new api.Element(control.container.find('.edit-menu-item-target'));
        control.elements.classes = new api.Element(
          control.container.find('.edit-menu-item-classes')
        );
        control.elements.xfn = new api.Element(control.container.find('.edit-menu-item-xfn'));
        control.elements.description = new api.Element(
          control.container.find('.edit-menu-item-description')
        );
        ...
        control.setting.bind(function (to, from) {
          ...
          if (false === to) {
            ...
            _.each(menuControl.getMenuItemControls(), function (otherControl) {
              if (
                from.menu_item_parent === otherControl.setting().menu_item_parent &&
                otherControl.setting().position > from.position
              ) {
        ...
        updateNotifications = function () {
          control.elements.url.element.toggleClass(
            'invalid',
            control.setting.notifications.has('invalid_url')
          );
        };
      ...
      _setupRemoveUI: function () {
        ...
        $removeBtn.on('click', function () {
          ...
          if ($next.length) {
            $adjacentFocusTarget = $next
              .find(false === addingItems ? '.item-edit' : '.item-delete')
              .first();
          } else if ($prev.length) {
            $adjacentFocusTarget = $prev
              .find(false === addingItems ? '.item-edit' : '.item-delete')
              .first();
          } else {
            $adjacentFocusTarget = control.container
              .nextAll('.customize-control-nav_menu')
              .find('.add-new-menu-item')
              .first();
          }
          ...
          _.each(addedItems, function (addedItem) {
            ...
            matches = addedItem
              .getAttribute('id')
              .match(/^customize-control-nav_menu_item-(-?\d+)$/, '');
            if (!matches) {
              return;
            }
            ...
            if (
              menuItemControl &&
              deleteItemOriginalItemId == menuItemControl.params.original_item_id
            ) {
              instanceCounter++;
            }
          });
      ...
      _setupTitleUI: function () {
        ...
        control.setting.bind(function (item) {
          ...
          if (trimmedTitle || item.original_title) {
            titleEl.text(titleText).removeClass('no-title');
          } else {
            titleEl.text(titleText).addClass('no-title');
          }
        });
      },
      ...
      renderContent: function () {
        ...
        if (settingValue._invalid) {
          ...
          control.params.title = api.Menus.data.l10n.invalidTitleTpl.replace(
            '%s',
            control.params.title
          );
        ...
        control.params.classes = _.isArray(settingValue.classes)
          ? settingValue.classes.join(' ')
          : settingValue.classes;
      ...
      onChangeExpanded: function (showOrHide, params) {
        ...
        if (showOrHide) {
          ...
          complete = function () {
            $menuitem.removeClass('menu-item-edit-inactive').addClass('menu-item-edit-active');
          ...
          complete = function () {
            $menuitem.addClass('menu-item-edit-inactive').removeClass('menu-item-edit-active');
      ...
      focus: function (params) {
        params = params || {};
        var control = this,
          originalCompleteCallback = params.completeCallback,
          focusControl;
      ...
      _changePosition: function (offset) {
        ...
        if (
          (realPosition === 0 && offset < 0) ||
          (realPosition === siblingSettings.length - 1 && offset > 0)
        ) {
        ...
        if (adjacentSetting) {
          adjacentSetting.set(
            $.extend(_.clone(adjacentSetting()), {
      ...
      _changeDepth: function (offset) {
        ...
        if (-1 === offset) {
          ...
          _(siblingControls)
            .chain()
            .slice(realPosition)
            .each(function (siblingControl, i) {
              siblingControl.setting.set(
                $.extend({}, siblingControl.setting(), {
          ...
          _(control.getMenuControl().getMenuItemControls()).each(function (otherControl) {
            ...
            isControlToBeShifted =
              otherControl.setting().menu_item_parent ===
                parentControl.setting().menu_item_parent &&
            ...
            if (isControlToBeShifted) {
              ...
              otherControl.setting.set(
                $.extend(otherControlSettingValue, {
          ...
          _(control.getMenuControl().getMenuItemControls()).each(function (otherControl) {
            if (otherControl.setting().menu_item_parent === settingValue.menu_item_parent) {
              settingValue.position = Math.max(
                settingValue.position,
                otherControl.setting().position
              );
            }
          });
  ...
  api.Menus.MenuNameControl = api.Control.extend(
    /** @lends wp.customize.Menus.MenuNameControl.prototype */ {
      ready: function () {
        var control = this;
  ...
  api.Menus.MenuLocationsControl = api.Control.extend(
    /** @lends wp.customize.Menus.MenuLocationsControl.prototype */ {
      /**
       * Set up the control.
       *
       * @since 4.9.0
          ...
          var container = $(this),
            ...
            updateSelectedMenuLabel = function (selectedMenuId) {
              ...
              if (!selectedMenuId || !menuSetting || !menuSetting()) {
                ...
                container
                  .find('.theme-location-set')
                  .show()
                  .find('span')
                  .text(displayNavMenuName(menuSetting().name));
              }
            };
  ...
  api.Menus.MenuAutoAddControl = api.Control.extend(
    /** @lends wp.customize.Menus.MenuAutoAddControl.prototype */ {
      ready: function () {
        var control = this,
          settingValue = control.setting();
        ...
        control.autoAddElement = new api.Element(
          control.container.find('input[type=checkbox].auto_add')
        );
  ...
  api.Menus.MenuControl = api.Control.extend(
    /** @lends wp.customize.Menus.MenuControl.prototype */ {
      /**
       * Set up the control.
       */
      ready: function () {
        ...
        if (menu) {
          ...
          api.control.each(function (widgetControl) {
            if (
              !widgetControl.extended(api.controlConstructor.widget_form) ||
          ...
          widgetTemplate = $(
            '#available-widgets-list .widget-tpl:has( input.id_base[ value=nav_menu ] )'
          );
        ...
        menuList.on('sortstop', function () {
          setTimeout(function () {
            ...
            _.each(menuItemControls, function (menuItemControl) {
              ...
              setting.menu_item_parent = parseInt(
                menuItemControl.container.find('.menu-item-data-parent-id').val(),
                10
              );
      ...
      _handleDeletion: function () {
        ...
        api.control.each(function (widgetControl) {
          ...
          widgetControl.container
            .find('.nav-menu-widget-form-controls:first')
            .toggle(0 !== navMenuCount);
          widgetControl.container
            .find('.nav-menu-widget-no-menus-message:first')
            .toggle(0 === navMenuCount);
      ...
      reflowMenuItems: function () {
        ...
        reflowRecursively = function (context) {
          ...
          _.each(currentMenuItemControls, function (menuItemControl) {
            ...
            // Update depth.
            if (
              !menuItemControl.container.hasClass('menu-item-depth-' + String(context.currentDepth))
            ) {
              _.each(
                menuItemControl.container.prop('className').match(/menu-item-depth-\d+/g),
                function (className) {
          ...
          if (currentMenuItemControls.length) {
            _(currentMenuItemControls).each(function (menuItemControl) {
              menuItemControl.container.removeClass(
                'move-up-disabled move-down-disabled move-left-disabled move-right-disabled'
              );
      ...
      addItemToMenu: function (item) {
        ...
        item = $.extend({}, api.Menus.data.defaultSettingValues.nav_menu_item, item, {
  ...
  api.Menus.applySavedData = function (data) {
    ...
    _(data.nav_menu_updates).each(function (update) {
      ...
      if ('inserted' === update.status) {
        ...
        api.control.each(function (setting) {
          if (
            !setting.extended(api.controlConstructor.widget_form) ||
        ...
        wp.customize.control.each(function (control) {
          if (/^nav_menu_locations\[/.test(control.id)) {
            control.container
              .find('option[value=' + String(update.previous_term_id) + ']')
              .remove();
          }
        });
        ...
        api.each(function (setting) {
          ...
          if (
            /^nav_menu_locations\[/.test(setting.id) &&
            setting.get() === update.previous_term_id
          ) {
    ...
    _(data.nav_menu_item_updates).each(function (update) {
      ...
      if ('inserted' === update.status) {
        ...
        newControl.container.toggleClass(
          'menu-item-edit-inactive',
          oldControl.container.hasClass('menu-item-edit-inactive')
        );
      }
    });
