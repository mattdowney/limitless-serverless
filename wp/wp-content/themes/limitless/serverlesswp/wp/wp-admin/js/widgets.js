  ...
  window.wpWidgets = {
    ...
    init: function () {
      ...
      $(window).on('beforeunload.widgets', function (event) {
        ...
        if (0 !== dirtyWidgetIds.length) {
          unsavedWidgetsElements = $('#widgets-right')
            .find('.widget')
            .filter(function () {
              return (
                -1 !==
                dirtyWidgetIds.indexOf(
                  $(this)
                    .prop('id')
                    .replace(/^widget-\d+_/, '')
                )
      ...
      $('#widgets-left .sidebar-name').on('click', function () {
        ...
        $wrap
          .toggleClass('closed')
          .find('.handlediv')
          .attr('aria-expanded', !$wrap.hasClass('closed'));
      ...
      sidebars.droppable({
        ...
        over: function (event) {
          ...
          if ($wrap.hasClass('closed')) {
            ...
            $wrap.removeClass('closed').find('.handlediv').attr('aria-expanded', 'true');
          }
      ...
      sidebars
        .sortable({
          ...
          receive: function (event, ui) {
            ...
            if (
              $sender.attr('id').indexOf('orphaned_widgets') > -1 &&
      ...
      $('#available-widgets .widget .widget-top').on('click.widgets-chooser', function () {
        ...
        if ($widget.hasClass('widget-in-question') || $('#widgets-left').hasClass('chooser')) {
          ...
          chooserButtons.on('click.widgets-chooser', function () {
            ...
            $(this).attr('aria-pressed', 'true').closest('li').addClass('widgets-chooser-selected');
          });
        }
      });
      ...
      // Add event handlers.
      chooser
        .on('click.widgets-chooser', function (event) {
    ...
    saveOrder: function (sidebarId) {
      ...
      $.post(ajaxurl, data, function () {
        $('#inactive-widgets-control-remove').prop(
          'disabled',
          !$('#wp_inactive_widgets .widget').length
        );
    ...
    removeInactiveWidgets: function () {
      var $element = $('.remove-inactive-widgets'),
        self = this,
        a,
        data;
    ...
    addWidget: function (chooser) {
      ...
      window.setTimeout(function () {
        ...
        window.wp.a11y.speak(
          wp.i18n.__('Widget has been added to the selected sidebar'),
          'assertive'
        );
      }, 250);
    },
    ...
    closeSidebar: function (event) {
      this.hoveredSidebar.addClass('closed').find('.handlediv').attr('aria-expanded', 'false');
