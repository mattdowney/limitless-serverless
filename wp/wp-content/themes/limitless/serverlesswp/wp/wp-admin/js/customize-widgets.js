...
(function (wp, $) {
  ...
  api.Widgets.WidgetModel = Backbone.Model.extend(
    /** @lends wp.customize.Widgets.WidgetModel.prototype */ {
      id: null,
      temp_id: null,
      classname: null,
      control_tpl: null,
  ...
  api.Widgets.WidgetCollection = Backbone.Collection.extend(
    /** @lends wp.customize.Widgets.WidgetCollection.prototype */ {
  ...
  api.Widgets.availableWidgets = new api.Widgets.WidgetCollection(
    api.Widgets.data.availableWidgets
  );
  ...
  api.Widgets.SidebarModel = Backbone.Model.extend(
    /** @lends wp.customize.Widgets.SidebarModel.prototype */ {
      after_title: null,
      after_widget: null,
      before_title: null,
      before_widget: null,
  ...
  api.Widgets.SidebarCollection = Backbone.Collection.extend(
    /** @lends wp.customize.Widgets.SidebarCollection.prototype */ {
      model: api.Widgets.SidebarModel,
    }
  );
  api.Widgets.registeredSidebars = new api.Widgets.SidebarCollection(
    api.Widgets.data.registeredSidebars
  );

  api.Widgets.AvailableWidgetsPanelView = wp.Backbone.View.extend(
    /** @lends wp.customize.Widgets.AvailableWidgetsPanelView.prototype */ {
      ...
      initialize: function () {
         ...
         */
        $('#customize-controls, #available-widgets .customize-section-title').on(
          'click keydown',
          function (e) {
      ...
      keyboardAccessible: function (event) {
        ...
        if (
          ...
          isTab &&
          ((isShift && isSearchFocused) || (!isShift && isLastWidgetFocused))
        ) {
          this.currentSidebarControl.container.find('.add-new-widget').focus();
  ...
  api.Widgets.WidgetControl = api.Control.extend(
    /** @lends wp.customize.Widgets.WidgetControl.prototype */ {
      ...
      _setupWideWidget: function () {
        ...
        if (
          !this.params.is_wide ||
          $(window).width() <= 640 /* max-width breakpoint in customize-controls.css */
        ) {
          return;
        }
      ...
      _setupReorderUI: function () {
        ...
        updateAvailableSidebars = function () {
          var $sidebarItems = $moveWidgetArea.find('li'),
            selfSidebarItem,
        ...
        $reorderNav
          .find('.move-widget, .move-widget-down, .move-widget-up')
          .each(function () {
          ...
          .on('click keypress', function (event) {
            ...
            if ($(this).is('.move-widget')) {
              ...
              if (
                (isMoveUp && i === 0) ||
                (isMoveDown && i === self.getSidebarWidgetsControl().setting().length - 1)
              ) {
                return;
              }
      ...
      _setupRemoveUI: function () {
        ...
        $removeBtn.on('click', function () {
          ...
          if (self.container.next().is('.customize-control-widget_form')) {
            ...
            $adjacentFocusTarget = self.container
              .next('.customize-control-sidebar_widgets')
              .find('.add-new-widget:first');
          }

          self.container.slideUp(function () {
            var sidebarsWidgetsControl = api.Widgets.getSidebarWidgetControlContainingWidget(
                self.params.widget_id
              ),
              sidebarWidgetIds,
              i;
      ...
      updateWidget: function (args) {
        ...
        jqxhr.done(function (r) {
          ...
          if (r.success) {
            ...
            hasSameInputsInResponse =
              self._getInputsSignature($inputs) === self._getInputsSignature($sanitizedInputs);
            ...
            if (hasSameInputsInResponse && self.liveUpdateMode) {
              $inputs.each(function (i) {
                ...
                canUpdateState =
                  !_.isEqual(submittedState, sanitizedState) &&
                  (args.ignoreActiveElement || !$input.is(document.activeElement));
                if (canUpdateState) {
            ...
            if (completeCallback) {
              ...
              $widgetContent.prepend(
                '<p class="widget-error"><strong>' + message + '</strong></p>'
              );
            }
          }
        });
  ...
  api.Widgets.WidgetsPanel = api.Panel.extend(
    /** @lends wp.customize.Widgets.WigetsPanel.prototype */ {
      /**
  ...
  api.Widgets.SidebarSection = api.Section.extend(
    /** @lends wp.customize.Widgets.SidebarSection.prototype */ {
      /**
  ...
  api.Widgets.SidebarControl = api.Control.extend(
    /** @lends wp.customize.Widgets.SidebarControl.prototype */ {
      /**
       * Set up the control
       */
      ready: function () {
      ...
      _setupModel: function () {
        ...
        this.setting.bind(function (newWidgetIds, oldWidgetIds) {
          ...
          _(removedWidgetIds).each(function (removedWidgetId) {
            ...
            setTimeout(function () {
              var removedControl,
                wasDraggedToAnotherSidebar,
                inactiveWidgets,
                removedIdBase,
                widget,
                isPresentInAnotherSidebar = false;

              // Check if the widget is in another sidebar.
              api.each(function (otherSetting) {
                if (
                  otherSetting.id === self.setting.id ||
                  0 !== otherSetting.id.indexOf('sidebars_widgets[') ||
                  otherSetting.id === 'sidebars_widgets[wp_inactive_widgets]'
                ) {
                  return;
                }

                var otherSidebarWidgets = otherSetting(),
                  i;
              ...
              wasDraggedToAnotherSidebar =
                removedControl &&
                $.contains(document, removedControl.container[0]) &&
                !$.contains(self.$sectionContent[0], removedControl.container[0]);
      ...
      _setupSortable: function () {
        ...
        this.$sectionContent.sortable({
          ...
          update: function () {
            var widgetContainerIds = self.$sectionContent.sortable('toArray'),
              widgetIds;
                ...
                api.section.each(function (otherSection) {
                  if (otherSection.container.find('.customize-control-sidebar_widgets').length) {
                    otherSection.container
                      .find('.accordion-section-content:first')
      ...
      _applyCardinalOrderClassNames: function () {
        ...
        if (
          0 === widgetControls.length ||
          (1 === api.Widgets.registeredSidebars.length && widgetControls.length <= 1)
        ) {
        ...
        $(widgetControls).each(function () {
          $(this.container)
            ...
            .find('.move-widget-down, .move-widget-up')
            .prop('tabIndex', 0);
        });

        _.first(widgetControls)
          .container.addClass('first-widget')
          .find('.move-widget-up')
          .prop('tabIndex', -1);

        _.last(widgetControls)
          .container.addClass('last-widget')
          .find('.move-widget-down')
          .prop('tabIndex', -1);
      },
      ...
      addWidget: function (widgetId) {
        ...
        controlHtml = $('#widget-tpl-' + widget.get('id'))
          .html()
          .trim();
        ...
        controlContainer.attr(
          'id',
          'customize-control-' + settingId.replace(/\]/g, '').replace(/\[/g, '-')
        );
        ...
        if (!isExistingWidget) {
          settingArgs = {
            transport: api.Widgets.data.selectiveRefreshableWidgets[widget.get('id_base')]
              ? 'postMessage'
              : 'refresh',
  ...
  ((api.Widgets.highlightWidgetFormControl = function (widgetId) {
    ...
    (api.Widgets.getSidebarWidgetControlContainingWidget = function (widgetId) {
      ...
      api.control.each(function (control) {
        if (
          control.params.type === 'sidebar_widgets' &&
          -1 !== _.indexOf(control.setting(), widgetId)
        ) {
  ...
  $(document).on('widget-added', function (event, widgetContainer) {
    ...
    parsedWidgetId = parseWidgetId(
      widgetContainer.find('> .widget-inside > .form > .widget-id').val()
    );
  ...
  function widgetIdToSettingId(widgetId) {
    var parsed = parseWidgetId(widgetId),
      settingId;
