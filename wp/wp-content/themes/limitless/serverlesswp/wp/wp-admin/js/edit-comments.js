...
(function ($) {
  ...
  updateInModerationText = function (response) {
    ...
    if (isDashboard && response.in_moderation) {
      $('.comment-mod-count', '#dashboard_right_now')[
        response.in_moderation > 0 ? 'removeClass' : 'addClass'
      ]('hidden');
    }
  };
  ...
  updateHtmlTitle = function (diff) {
    ...
    titleRegEx =
      titleRegEx ||
      new RegExp(__('Comments (%s)').replace('%s', '\\([0-9' + thousandsSeparator + ']+\\)') + '?');
    ...
    if (titleCount >= 1) {
      ...
      if (regExMatch) {
        /* translators: %s: Comments count. */
        newTitle = document.title.replace(
          regExMatch[0],
          __('Comments (%s)').replace('%s', titleDiv.text()) + ' '
        );
      }
    } else {
  ...
  window.setCommentsList = function () {
    ...
    updateTotalCount = function (total, time, setConfidentTime) {
      if (time < lastConfidentTime) return;

      if (setConfidentTime) lastConfidentTime = time;
    ...
    dimAfter = function (r, settings) {
      ...
      if (c.is('.unapproved')) {
        if (settings.data.id == replyID) replyButton.text(__('Approve and Reply'));

        c.find('.row-actions span.view')
          .addClass('hidden')
          .end()
          .find('div.comment_status')
          .html('0');
      } else {
        if (settings.data.id == replyID) replyButton.text(__('Reply'));
    ...
    delBefore = function (settings, list) {
      ...
      if (wpListsData.indexOf(':trash=1') != -1) action = 'trash';
      else if (wpListsData.indexOf(':spam=1') != -1) action = 'spam';
      ...
      if (action) {
        ...
        if (el.siblings('#replyrow').length && commentReply.cid == id) commentReply.close();
        ...
        if (el.is('tr')) {
          ...
          h = $(
            '<div id="undo-' +
              id +
              '" style="display:none;" class="undo un' +
              action +
              '">' +
              note +
              '</div>'
        ...
        a.attr(
          'href',
          'comment.php?action=un' +
            action +
            'comment&c=' +
            id +
            '&_wpnonce=' +
            settings.data._ajax_nonce
        );
        ...
        $('.avatar', el)
          .first()
          .clone()
          .prependTo('#undo-' + id + ' .' + action + '-undo-inside');
    ...
    delAfter = function (r, settings) {
      ...
      if (!isDashboard) {
        ...
        if ($(settings.target).parent().is('span.undo')) total++;
        else total--;
        ...
        if ('object' === typeof r) {
          if (
            response.supplemental.total_items_i18n &&
            lastConfidentTime < response.supplemental.time
          ) {
            total_items_i18n = response.supplemental.total_items_i18n || '';
            if (total_items_i18n) {
              $('.displaying-num').text(
                total_items_i18n.replace('&nbsp;', String.fromCharCode(160))
              );
              $('.total-pages').text(
                response.supplemental.total_pages_i18n.replace('&nbsp;', String.fromCharCode(160))
              );
              $('.tablenav-pages')
                .find('.next-page, .last-page')
                .toggleClass(
                  'disabled',
                  response.supplemental.total_pages == $('.current-page').val()
                );
            }
      ...
      if (
        !theExtraList ||
        theExtraList.length === 0 ||
        theExtraList.children().length === 0 ||
        undoing
      ) {
        return;
      }
    ...
    refillTheExtraList = function (ev) {
      ...
      if (true === args.comment_type) args.comment_type = '';
     ...
     * @global
     */
    window.theExtraList = $('#the-extra-comment-list').wpList({
      alt: '',
      delColor: 'none',
      addColor: 'none',
    });
     ...
     * @global
     */
    window.theList = $('#the-comment-list')
      .wpList({
        alt: '',
        delBefore: delBefore,
        dimAfter: dimAfter,
      ...
      .on('wpListDelEnd', function (e, s) {
        var wpListsData = $(s.target).attr('data-wp-lists'),
          id = s.element.replace(/[^0-9]+/g, '');
  ...
  window.commentReply = {
    ...
    init: function () {
      ...
      $('input#author-name, input#author-email, input#author-url', row).on(
        'keypress',
        function (e) {
          if (e.which == 13) {
            commentReply.send();
      ...
      $('#doaction, #post-query-submit').on('click', function () {
        if ($('#the-comment-list #replyrow').length > 0) commentReply.close();
      });
     ...
     */
    revert: function () {
      if ($('#the-comment-list #replyrow').length < 1) return false;
    ...
    close: function () {
      ...
      if ('edit-comment' === this.act) {
        commentRow
          .fadeIn(300, function () {
            commentRow.show().find('.vim-q').attr('aria-expanded', 'false').trigger('focus');
          })
          .css('backgroundColor', '');
      }
      ...
      if ('replyto-comment' === this.act) {
        commentRow.find('.vim-r').attr('aria-expanded', 'false').trigger('focus');
      }

      // Reset the Quicktags buttons.
      if (typeof QTags != 'undefined') QTags.closeAllTags('replycontent');
      ...
      $('#edithead input').val('');
      $('.notice-error', replyRow).addClass('hidden').find('.error').empty();
    ...
    open: function (comment_id, post_id, action) {
      ...
      setTimeout(function () {
        ...
        if (scrollBottom - 20 < rbottom) window.scroll(0, rbottom - vp + 35);
        else if (rtop - 20 < scrollTop) window.scroll(0, rtop - 35);
    ...
    send: function () {
      ...
      if ($('#comment-' + $('#comment_ID').val()).hasClass('unapproved')) post.approve_parent = 1;
    ...
    show: function (xml) {
      ...
      if ('edit-comment' == t.act) $(id).remove();
      ...
      id = $(id);
      t.addEvents(id);
      bg = id.hasClass('unapproved')
        ? '#FFFFE0'
        : id.closest('.widefat, .postbox').css('backgroundColor');

      id.animate({ backgroundColor: '#CCEEBB' }, 300).animate(
        { backgroundColor: bg },
        300,
        function () {
          if (pid && pid.length) {
    ...
    error: function (r) {
      ...
      if (r.responseText) er = r.responseText.replace(/<.[^<>]*?>/g, '');
    ...
    if (typeof $.table_hotkeys != 'undefined') {
      ...
      make_hotkeys_redirect = function (which) {
        return function () {
          ...
          if (l.length)
            window.location =
              l[0].href.replace(/\&hotkeys_highlight_(first|last)=1/g, '') +
              '&hotkeys_highlight_' +
              first_last +
              '=1';
        };
