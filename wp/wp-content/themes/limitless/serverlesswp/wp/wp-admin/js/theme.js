...
(function ($) {
  ...
  themes.Collection = Backbone.Collection.extend({
    ...
    search: function (term) {
      ...
      results = this.filter(function (data) {
        ...
        description = data.get('description').replace(/(<([^>]+)>)/gi, '');
  ...
  themes.view.Theme = wp.Backbone.View.extend({
    ...
    addFocus: function () {
      var $themeToFocus = $(':focus').hasClass('theme')
        ? $(':focus')
        : $(':focus').parents('.theme');
    ...
    setNavButtonsState: function () {
      ...
      if (0 === this.model.collection.indexOf(current)) {
        previousThemeButton.addClass('disabled').prop('disabled', true);
      ...
      if (_.isUndefined(this.model.collection.at(this.model.collection.indexOf(current) + 1))) {
        nextThemeButton.addClass('disabled').prop('disabled', true);
  ...
  themes.view.Details = wp.Backbone.View.extend({
    ...
    collapse: function (event) {
      ...
      if (
        $(event.target).is('.theme-backdrop') ||
    ...
    navigation: function () {
      ...
      if (this.model.cid === this.model.collection.at(0).cid) {
        this.$el.find('.left').addClass('disabled').prop('disabled', true);
      }
      if (this.model.cid === this.model.collection.at(this.model.collection.length - 1).cid) {
        this.$el.find('.right').addClass('disabled').prop('disabled', true);
      }
    },
      ...
      $(document).one('wp-theme-delete-success', function (event, response) {
        ...
        $('[data-slug="' + response.slug + '"]')
          ...
          .fadeOut(350, function () {
            $(this).remove();
            _themes.data.themes = _.without(
              _themes.data.themes,
              _.findWhere(_themes.data.themes, { id: response.slug })
            );
  ...
  themes.view.Preview = themes.view.Details.extend({
    ...
    render: function () {
      ...
      themes.router.navigate(
        themes.router.baseUrl(themes.router.themePath + this.model.get('id')),
        { replace: false }
      );
    ...
    togglePreviewDeviceButtons: function (newDevice) {
      ...
      $devices.find('button').removeClass('active').attr('aria-pressed', false);
      ...
      if (!themes.isInstall && themes.data.settings.canInstall) {
        this.$el.append(
          '<div class="theme add-new-theme"><a href="' +
            ...
            '"><div class="theme-screenshot"><span></span></div><h2 class="theme-name">' +
            l10n.addNew +
            '</h2></a></div>'
        );
      }
  ...
  themes.view.Search = wp.Backbone.View.extend({
    ...
    doSearch: function (event) {
      ...
      if (event.target.value) {
        themes.router.navigate(
          themes.router.baseUrl(themes.router.searchPath + event.target.value),
          options
        );
      } else {
        themes.router.navigate(themes.router.baseUrl(''));
    ...
    pushState: function (event) {
      ...
      if (event.target.value) {
        url = themes.router.baseUrl(
          themes.router.searchPath + encodeURIComponent(event.target.value)
        );
      }
  ...
  themes.view.InstallerSearch = themes.view.Search.extend({
    ...
    doSearch: function (value) {
      ...
      $('.filter-links li > a.current').removeClass('current').removeAttr('aria-current');
      ...
      // Set route.
      themes.router.navigate(
        themes.router.baseUrl(themes.router.searchPath + encodeURIComponent(value)),
        { replace: true }
      );
    },
  });
  ...
  themes.view.Installer = themes.view.Appearance.extend({
    ...
    onFilter: function (event) {
      ...
      $('.filter-links li > a, .theme-section')
        .removeClass(this.activeClass)
        .removeAttr('aria-current');
      $el.addClass(this.activeClass).attr('aria-current', 'page');
  ...
  $(function () {
    ...
    $(document.body).on('click', '.load-customize', function () {
      ...
      urlParser.search = $.param(
        _.extend(wp.customize.utils.parseQueryString(urlParser.search.substr(1)), {
