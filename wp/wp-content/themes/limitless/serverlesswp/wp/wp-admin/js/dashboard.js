...
jQuery(function ($) {
  ...
  window.ajaxPopulateWidgets = function (el) {
    ...
    function show(i, id) {
      var p,
        e = $('#' + id + ' div.inside:visible').find('.widget-loading');
      ...
      if (e.length) {
        ...
        setTimeout(function () {
          ...
          p.load(
            ajaxurl + '?action=dashboard-widgets&widget=' + id + '&pagenow=' + pagenow,
            '',
            function () {
  ...
  window.quickPressLoad = function () {
    ...
    $('#quick-press .submit input[type="submit"], #quick-press .submit input[type="reset"]').prop(
      'disabled',
      false
    );
  ...
  function autoResizeTextarea() {
    ...
    editor.on('focus input propertychange', function () {
      var $this = $(this),
        ...
        cloneHeight =
          clone.css('width', $this.css('width')).text(textareaContent).outerHeight() + 2;
      ...
      if (
        ...
        (cloneHeight >= editorMaxHeight && editorHeight >= editorMaxHeight)
      ) {
        return;
      }
  ...
  app = window.wp.communityEvents = /** @lends wp.communityEvents */ {
    ...
    renderEventsTemplate: function (templateParams, initiatedBy) {
      ...
      if (
        ...
        (templateParams.location.ip || templateParams.location.latitude)
      ) {
    ...
    getTimeZoneAbbreviation: function (startTimestamp) {
      ...
      if ('undefined' === typeof timeZoneAbbreviation) {
        ...
        timeZoneAbbreviation =
          _x('GMT', 'Events widget offset prefix') + sign + timeZoneOffset / 60;
      }
