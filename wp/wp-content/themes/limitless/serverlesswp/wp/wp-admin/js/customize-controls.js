...
(function (exports, $) {
  ...
  api.OverlayNotification = api.Notification.extend(
    /** @lends wp.customize.OverlayNotification.prototype */ {
      /**
  ...
  api.Notifications = api.Values.extend(
    /** @lends wp.customize.Notifications.prototype */ {
      /**
      ...
      get: function (args) {
        ...
        params = _.extend({ sort: false }, args);
      ...
      render: function () {
        var collection = this,
          notifications,
          hadOverlayNotification = false,
          hasOverlayNotification,
        ...
        if (
          collection.container.is(collection.previousContainer) &&
          _.isEqual(notifications, collection.previousNotifications)
        ) {
          return;
        }
        ...
        _.each(notifications, function (notification) {
          ...
          if (
            wp.a11y &&
            (!previousNotificationsByCode[notification.code] ||
              !_.isEqual(
                notification.message,
                previousNotificationsByCode[notification.code].message
              ))
          ) {
        ...
        if (collection.previousNotifications) {
          hadOverlayNotification = Boolean(
            _.find(collection.previousNotifications, function (notification) {
        ...
        if (hasOverlayNotification) {
          collection.focusContainer =
            overlayNotifications[overlayNotifications.length - 1].container;
      ...
      constrainFocus: function constrainFocus(event) {
        ...
        if (
          !$.contains(collection.focusContainer[0], event.target) ||
          !$.contains(collection.focusContainer[0], document.activeElement)
        ) {
  ...
  api.Setting = api.Value.extend(
    /** @lends wp.customize.Setting.prototype */ {
      /**
       * Default params.
       *
       * @since 4.9.0
      ...
      initialize: function (id, value, options) {
        var setting = this,
          params;
        params = _.extend({ previewer: api.previewer }, setting.defaults, options || {});
  ...
  api.dirtyValues = function dirtyValues(options) {
    ...
    api.each(function (setting) {
      ...
      if (
        api.state('changesetStatus').get() &&
        options &&
        options.unsaved &&
        (_.isUndefined(settingRevision) || settingRevision <= api._lastSavedRevision)
      ) {
        return;
      }
  ...
  api.requestChangesetUpdate = function requestChangesetUpdate(changes, args) {
    ...
    _.each(api.dirtyValues({ unsaved: true }), function (dirtyValue, settingId) {
      if (!changes || null !== changes[settingId]) {
        submittedChanges[settingId] = _.extend({}, submittedChanges[settingId] || {}, {
          value: dirtyValue,
        });
      }
    });
    ...
    if (
      !submittedArgs.force &&
      _.isEmpty(submittedChanges) &&
      null === submittedArgs.title &&
      null === submittedArgs.date
    ) {
      deferred.resolve({});
      return deferred.promise();
    }
    ...
    request.done(function requestChangesetUpdateDone(data) {
      ...
      if (data.setting_validities) {
        _.each(data.setting_validities, function (validity, settingId) {
          if (
            true === validity &&
            _.isObject(submittedChanges[settingId]) &&
            !_.isUndefined(submittedChanges[settingId].value)
          ) {
      ...
      api.previewer.send(
        'changeset-saved',
        _.extend({}, data, { saved_changeset_values: savedChangesetValues })
      );
    });
  ...
  focus = function (params) {
    ...
    focus = function () {
      ...
      var focusContainer;
      if (
        (construct.extended(api.Panel) || construct.extended(api.Section)) &&
        construct.expanded &&
        construct.expanded()
      ) {
      ...
      if (0 === focusElement.length) {
        ...
        focusElement = focusContainer
          .find('input, select, textarea, button, object, a[href], [tabindex]')
          .filter(':visible')
          .first();
      }
      focusElement.focus();
  ...
  api.utils.prioritySort = function (a, b) {
    if (
      ...
      typeof a.params.instanceNumber === 'number' &&
      typeof b.params.instanceNumber === 'number'
    ) {
  ...
  api.utils.areElementListsEqual = function (listA, listB) {
    var equal =
      ...
      -1 ===
        _.indexOf(
          _.map(
            // Are there any false values in the list returned by map?
          ...
          false
        ); // Check for presence of false in map's return value.
    return equal;
  };
  ...
  api.utils.getCurrentTimestamp = function getCurrentTimestamp() {
    ...
    timestampDifferential +=
      api.settings.initialClientTimestamp - api.settings.initialServerTimestamp;
  ...
  Container = api.Class.extend(
    /** @lends wp.customize~Container.prototype */ {
      ...
      initialize: function (id, options) {
        ...
        container.templateSelector =
          container.params.templateId ||
          'customize-' + container.containerType + '-' + container.params.type;
      ...
      getContainer: function () {
        ...
        if (template && container.container) {
          return template(_.extend({ id: container.id }, container.params))
            .toString()
            .trim();
        }
  ...
  api.Section = Container.extend(
    /** @lends wp.customize.Section.prototype */ {
      ...
      attachEvents: function () {
        ...
        section.container
          .find(
            '.accordion-section-title button, .customize-section-back, .accordion-section-title[tabindex]'
          )
        ...
        section.container
          .find('.customize-section-title .customize-help-toggle')
          .on('click', function () {
      ...
      onChangeExpanded: function (expanded, args) {
        var section = this,
          ...
          sectionTitle = section.headContainer
            .find('.accordion-section-title button, .accordion-section-title[tabindex]')
            .first(),
          expand,
          panel;
  ...
  api.ThemesSection = api.Section.extend(
    /** @lends wp.customize.ThemesSection.prototype */ {
      currentTheme: '',
      overlay: '',
      template: '',
        ...
        inject = function (panelId) {
          ...
          api.panel(panelId, function (panel) {
            ...
            panel.deferred.embedded.done(function () {
              ...
              if (!section.headContainer.parent().is(parentContainer)) {
                parentContainer
                  .find('.customize-themes-full-container-container')
                  .before(section.headContainer);
              }
              if (!section.contentContainer.parent().is(section.headContainer)) {
                section.containerParent.append(section.contentContainer);
              }
      ...
      attachEvents: function () {
        ...
        section.contentContainer.on('click', '.feature-filter-toggle', function (e) {
          ...
          section.filtersHeight = $filterToggle
            .parents('.themes-filter-bar')
            .next('.filter-drawer')
            .height();
          ...
          if ($filterToggle.hasClass('open')) {
            ...
            section.contentContainer
              .find('.themes')
              .css('margin-top', section.filtersHeight + marginOffset);
          } else {
            ...
            if (otherSection !== section) {
              ...
              if ('themes' === otherSection.params.type) {
                ...
                if (
                  '' === searchTerm &&
                  '' !== section.term &&
                  'local' !== section.params.filter_type
                ) {
                  section.term = '';
          ...
          if (0 !== themes.length) {
            ...
            if (1 === page) {
              ...
              _.each(section.controls().slice(0, 3), function (control) {
                var img,
                  src = control.params.theme.screenshot[0];
                if (src) {
                  img = new Image();
                  img.src = src;
                }
              ...
              if ('local' !== section.params.filter_type) {
                wp.a11y.speak(
                  api.settings.l10n.themeSearchResults.replace('%d', data.info.results)
                );
              }
            }
      ...
      loadControls: function (themes, page) {
        ...
        _.each(themes, function (theme) {
          var themeControl = new api.controlConstructor.theme(
            section.params.action + '_theme_' + theme.id,
            {
              type: 'theme',
      ...
      filterSearch: function (term) {
        var count = 0,
          ...
          section = this,
          noFilter =
            api.section.has('wporg_themes') && 'remote' !== section.params.filter_type
              ? '.no-themes-local'
              : '.no-themes',
          controls = section.controls(),
          terms;
      ...
      filtersChecked: function () {
        ...
        if (0 === tags.length) {
          ...
          section.contentContainer
            .find('.feature-filter-toggle .theme-filter-count')
            .text(tags.length);
      ...
      showDetails: function (theme, callback) {
        var section = this,
          panel = api.panel('themes');
        section.currentTheme = theme.id;
        section.overlay.html(section.template(theme)).fadeIn('fast').focus();
        ...
        function disableInstallButtons() {
          return (
            ...
            false === api.settings.theme._canInstall ||
            true === api.settings.theme._filesystemCredentialsNeeded
          );
        }

        section.overlay
          .find('button.preview, button.preview-theme')
          .toggleClass('disabled', disableSwitchButtons());
        section.overlay
          .find('button.theme-install')
          .toggleClass('disabled', disableInstallButtons());
      ...
      closeDetails: function () {
        ...
        api
          .control(section.params.action + '_theme_' + section.currentTheme)
          .container.find('.theme')
          .focus();
      },
  ...
  api.OuterSection = api.Section.extend(
    /** @lends wp.customize.OuterSection.prototype */ {
      /**
  ...
  api.Panel = Container.extend(
    /** @lends wp.customize.Panel.prototype */ {
      containerType: 'panel',
      ...
      onChangeExpanded: function (expanded, args) {
        ...
        var panel = this,
          ...
          topPanel = panel.headContainer.find(
            '.accordion-section-title button, .accordion-section-title[tabindex]'
          ),
        ...
        if (expanded && !accordionSection.hasClass('current-panel')) {
          ...
          if (
            ...
            1 === childSections.length &&
            childSections[0].active.get()
          ) {
      ...
      renderContent: function () {
        ...
        if (template && panel.headContainer) {
          panel.contentContainer.html(template(_.extend({ id: panel.id }, panel.params)));
        }
      },
    }
  );
  ...
  api.ThemesPanel = api.Panel.extend(
    /** @lends wp.customize.ThemsPanel.prototype */ {
      /**
      ...
      canSwitchTheme: function canSwitchTheme(slug) {
        ...
        return (
          'publish' === api.state('selectedChangesetStatus').get() &&
          ('' === api.state('changesetStatus').get() ||
            'auto-draft' === api.state('changesetStatus').get())
        );
      },
      ...
      attachEvents: function () {
        ...
        if (api.settings.theme._canInstall && api.settings.theme._filesystemCredentialsNeeded) {
          panel.notifications.add(
            new api.Notification('theme_install_unavailable', {
        ...
        function toggleDisabledNotifications() {
          if (panel.canSwitchTheme()) {
            ...
            panel.notifications.add(
              new api.Notification('theme_switch_unavailable', {
        ...
        panel.contentContainer.on(
          'click',
          '.customize-themes-section-title, .customize-themes-mobile-back',
          function () {
      ...
      onChangeExpanded: function (expanded, args) {
        ...
        if (expanded) {
          overlay
            ...
            .delay(200)
            .find('.customize-themes-full-container')
            .addClass('animate');
      ...
      installTheme: function (event) {
        ...
        if (preview) {
          api.notifications.add(
            new api.OverlayNotification('theme_installing', {
      ...
      loadThemePreview: function (themeId) {
        ...
        queryParams = _.extend(api.utils.parseQueryString(urlParser.search.substr(1)), {
          theme: themeId,
        ...
        api.notifications.add(
          new api.OverlayNotification('theme_previewing', {
  ...
  api.Control = api.Class.extend(
    /** @lends wp.customize.Control.prototype */ {
      defaultActiveArguments: { duration: 'fast', completeCallback: $.noop },
      ...
      linkElements: function () {
        ...
        nodes = control.container.find(
          '[data-customize-setting-link], [data-customize-setting-key-link]'
        );
        radios = {};
      ...
      embed: function () {
        ...
        inject = function (sectionId) {
          var parentContainer;
          if (!sectionId) {
            // @todo Allow a control to be embedded without a section, for instance a control embedded in the front end.
            return;
          }
          ...
          api.section(sectionId, function (section) {
            ...
            section.deferred.embedded.done(function () {
              parentContainer = section.contentContainer.is('ul')
                ? section.contentContainer
                : section.contentContainer.find('ul:first');
      ...
      getNotificationsContainerElement: function () {
        ...
        notificationsContainer = control.container.find(
          '.customize-control-notifications-container:first'
        );
      ...
      setupNotifications: function () {
        ...
        _.each(control.settings, function (setting) {
          ...
          setting.notifications.bind('add', function (settingNotification) {
            var params = _.extend({}, settingNotification, {
              setting: setting.id,
            });
            control.notifications.add(
              new api.Notification(setting.id + ':' + settingNotification.code, params)
            );
          });
        ...
        control.notifications.bind('rendered', function () {
          ...
          control.container.toggleClass(
            'has-error',
            0 !== _.where(notifications, { type: 'error' }).length
          );
        });
      ...
      renderNotifications: function () {
        ...
        if ('undefined' !== typeof console && console.warn) {
          console.warn(
            '[DEPRECATED] wp.customize.Control.prototype.renderNotifications() is deprecated in favor of instantiating a wp.customize.Notifications and calling its render() method.'
          );
        }
      ...
      addNewPage: function () {
        ...
        if (
          'dropdown-pages' !== control.params.type ||
  ...
  api.ColorControl = api.Control.extend(
    /** @lends wp.customize.ColorControl.prototype */ {
      ready: function () {
        var control = this,
  ...
  api.MediaControl = api.Control.extend(
    /** @lends wp.customize.MediaControl.prototype */ {
      /**
      ...
      ready: function () {
        ...
        api
          ...
          .container.on('expanded', function () {
            if (control.player) {
            ...
            wp.media
              ...
              .done(function () {
                ...
                wp.customize.previewer.send(
                  control.setting.id + '-attachment-data',
                  this.attributes
                );
              });
          }
  ...
  api.UploadControl = api.MediaControl.extend(
    /** @lends wp.customize.UploadControl.prototype */ {
      /**
       * Callback handler for when an attachment is selected in the media modal.
       * Gets the selected image information, and sets it within the control.
       */
  ...
  api.ImageControl = api.UploadControl.extend(
    /** @lends wp.customize.ImageControl.prototype */ {
      // @deprecated
  ...
  api.BackgroundControl = api.UploadControl.extend(
    /** @lends wp.customize.BackgroundControl.prototype */ {
      /**
       * When the control's DOM structure is ready,
       * set up internal event bindings.
       */
  ...
  api.BackgroundPositionControl = api.Control.extend(
    /** @lends wp.customize.BackgroundPositionControl.prototype */ {
      /**
      ...
      ready: function () {
        ...
        updateRadios = _.debounce(function () {
          ...
          radioInput = control.container.find(
            'input[name="background-position"][value="' + inputValue + '"]'
          );
  ...
  api.CroppedImageControl = api.MediaControl.extend(
    /** @lends wp.customize.CroppedImageControl.prototype */ {
      /**
      ...
      calculateImageSelectOptions: function (attachment, controller) {
        ...
        controller.set(
          ...
          control.hasRequiredAspectRatio(requiredRatio, realRatio)
        );
        controller.set('suggestedCropSize', {
          width: realWidth,
          height: realHeight,
          x1: 0,
          y1: 0,
  ...
  api.SiteIconControl = api.CroppedImageControl.extend(
    /** @lends wp.customize.SiteIconControl.prototype */ {
      /**
       * Create a media modal select frame, and store it so the instance can be reused when needed.
       */
      initFrame: function () {
  ...
  api.HeaderControl = api.Control.extend(
    /** @lends wp.customize.HeaderControl.prototype */ {
      ready: function () {
        ...
        api.HeaderTool.combinedList = api.HeaderTool.CombinedList = new api.HeaderTool.CombinedList(
          [api.HeaderTool.UploadsList, api.HeaderTool.DefaultsList]
        );
        ...
        wp.media.controller.Cropper.prototype.defaults.doCropArgs.customize_theme =
          api.settings.theme.stylesheet;
      },
      ...
      getInitialHeaderImage: function () {
        if (
          ...
          !api.get().header_image_data ||
          _.contains(
            ['remove-header', 'random-default-image', 'random-uploaded-image'],
            api.get().header_image
          )
        ) {
  ...
  api.ThemeControl = api.Control.extend(
    /** @lends wp.customize.ThemeControl.prototype */ {
      touchDrag: false,
  ...
  api.CodeEditorControl = api.Control.extend(
    /** @lends wp.customize.CodeEditorControl.prototype */ {
      /**
       * Initialize.
       *
       * @since 4.9.0
      ...
      initEditor: function () {
        ...
        if (
          wp.codeEditor &&
          (_.isUndefined(control.params.editor_settings) ||
            false !== control.params.editor_settings)
        ) {
          ...
          if (_.isObject(control.params.editor_settings)) {
            _.each(control.params.editor_settings, function (value, key) {
              if (_.isObject(value)) {
                editorSettings[key] = _.extend({}, editorSettings[key], value);
              }
            });
          }
        }
      ...
      focus: function (params) {
        var control = this,
          extendedParams = _.extend({}, params),
      ...
      initSyntaxHighlightingEditor: function (codeEditorSettings) {
        var control = this,
          $textarea = control.container.find('textarea'),
          settings,
      ...
      onTabPrevious: function onTabPrevious() {
        ...
        if (0 === controlIndex) {
          section.contentContainer
            .find(
              '.customize-section-title .customize-help-toggle, .customize-section-title .customize-section-description.open .section-description-close'
            )
            .last()
            .focus();
        } else {
      ...
      onUpdateErrorNotice: function onUpdateErrorNotice(errorAnnotations) {
        ...
        if (0 !== errorAnnotations.length) {
          ...
          control.setting.notifications.add(
            new api.Notification('csslint_error', {
              message: message,
              type: 'error',
            })
          );
  ...
  api.DateTimeControl = api.Control.extend(
    /** @lends wp.customize.DateTimeControl.prototype */ {
      /**
        ...
        control.container.find('.date-input').each(function () {
          ...
          input.on('change', function () {
            if (control.invalidDate) {
              control.notifications.add(
                new api.Notification('invalid_date', {
      ...
      validateInputs: function validateInputs() {
        ...
        if (control.inputElements.meridian && !control.invalidDate) {
          ...
          if (
            'am' !== control.inputElements.meridian.get() &&
            'pm' !== control.inputElements.meridian.get()
          ) {
        ...
        if (
          !control.section() ||
          (api.section.has(control.section()) && api.section(control.section()).expanded())
        ) {
      ...
      populateSetting: function populateSetting() {
        ...
        if (
          ...
          (!control.params.allowPastDate && !control.isFutureDate())
        ) {
          return false;
        }
      ...
      convertInputDateToString: function convertInputDateToString() {
        ...
        if (control.params.includeTime) {
          hourInTwentyFourHourFormat = control.inputElements.meridian
            ? control.convertHourToTwentyFourHourFormat(
                control.inputElements.hour(),
                control.inputElements.meridian()
              )
            : control.inputElements.hour();
          dateFormat = dateFormat.concat([
            ' ',
            pad(hourInTwentyFourHourFormat, 2),
            ':',
            'minute',
            ':',
            '00',
      ...
      convertHourToTwentyFourHourFormat: function convertHour(hourInTwelveHourFormat, meridian) {
        var hourInTwentyFourHourFormat,
          hour,
          midDayHour = 12;
  ...
  api.PreviewLinkControl = api.Control.extend(
    /** @lends wp.customize.PreviewLinkControl.prototype */ {
  ...
  api.PreviewFrame = api.Messenger.extend(
    /** @lends wp.customize.PreviewFrame.prototype */ {
      ...
      run: function (deferred) {
        ...
        params = _.extend(api.utils.parseQueryString(urlParser.search.substr(1)), {
  ...
  api.Previewer = api.Messenger.extend(
    /** @lends wp.customize.Previewer.prototype */ {
        ...
        previewer.add('previewUrl', params.previewUrl).setter(function (to) {
          ...
          _.find(parsedCandidateUrls, function (parsedCandidateUrl) {
            return !_.isUndefined(
              _.find(previewer.allowedUrls, function (allowedUrl) {
                parsedAllowedUrl.href = allowedUrl;
                if (
                  urlParser.protocol === parsedAllowedUrl.protocol &&
                  urlParser.host === parsedAllowedUrl.host &&
                  0 === urlParser.pathname.indexOf(parsedAllowedUrl.pathname.replace(/\/$/, ''))
                ) {
      ...
      cheatin: function () {
        $(document.body)
          ...
          .addClass('cheatin')
          .append(
            '<h1>' + api.l10n.notAllowedHeading + '</h1>' + '<p>' + api.l10n.notAllowed + '</p>'
          );
      },
    ...
    _.each(args.settingValidities, function (validity, settingId) {
      ...
      if (setting) {
        ...
        if (_.isObject(validity)) {
          _.each(validity, function (params, code) {
            ...
            if (existingNotification) {
              needsReplacement =
                notification.type !== existingNotification.type ||
                notification.message !== existingNotification.message ||
                !_.isEqual(notification.data, existingNotification.data);
            }
            if (needsReplacement) {
        ...
        setting.notifications.each(function (notification) {
          if (
            notification.fromServer &&
            'error' === notification.type &&
            (true === validity || !validity[notification.code])
          ) {
    ...
    if (args.focusInvalidControl) {
      ...
      _(_.values(invalidSettingControls)).find(function (controls) {
        return _(controls).find(function (control) {
          var isExpanded =
            control.section() &&
            api.section.has(control.section()) &&
            api.section(control.section()).expanded();
  ...
  api.reflowPaneContents = _.bind(function () {
    ...
    api.panel.each(function (panel) {
      ...
      appendContainer = panel.contentContainer.is('ul')
        ? panel.contentContainer
        : panel.contentContainer.find('ul:first');
      if (
        !api.utils.areElementListsEqual(sectionHeadContainers, appendContainer.children('[id]'))
      ) {
        _(sections).each(function (section) {
    ...
    api.section.each(function (section) {
      ...
      appendContainer = section.contentContainer.is('ul')
        ? section.contentContainer
        : section.contentContainer.find('ul:first');
  ...
  $(function () {
    ...
    api.bind('ready', function () {
      api.section.add(
        new api.OuterSection('publish_settings', {
    ...
    api.section('publish_settings', function (section) {
      ...
      api.control.add(
        new api.PreviewLinkControl('changeset_preview_link', {
          section: section.id,
          priority: 100,
        })
      );
    ...
    $('.customize-info')
      .find('> .accordion-section-title .customize-help-toggle')
      .on('click', function () {
    ...
    api.previewer = new api.Previewer(
      ...
      /** @lends wp.customize.previewer */ {
        ...
        query: function (options) {
           ...
           */
          queryVars.customized = JSON.stringify(
            api.dirtyValues({
        ...
        save: function (args) {
          ...
          submit = function () {
            var request,
              query,
              settingInvalidities = {},
              latestRevision = api._latestRevision,
            ...
            invalidControls = _.union(
              invalidSettingLessControls,
              _.values(api.findControlsForSettings(invalidSettings))
            );
            if (!_.isEmpty(invalidControls)) {
              ...
              if (invalidSettings.length) {
                api.notifications.add(
                  new api.Notification(errorCode, {
                    message: (1 === invalidSettings.length
                      ? api.l10n.saveBlockedError.singular
                      : api.l10n.saveBlockedError.plural
                    ).replace(/%s/g, String(invalidSettings.length)),
                    type: 'error',
                    dismissible: true,
                    saveFailure: true,
                  })
              ...
              deferred.rejectWith(previewer, [{ setting_invalidities: settingInvalidities }]);
            ...
            request.fail(function (response) {
              ...
              if ('invalid_nonce' === response) {
                ...
                if (
                  'not_future_date' === response.code &&
                  api.section.has('publish_settings') &&
                  api.section('publish_settings').active.get() &&
                  api.control.has('changeset_scheduled_date')
                ) {
                  api
                    .control('changeset_scheduled_date')
                    .toggleFutureDateNotification(true)
                    .focus();
                } else if ('changeset_locked' !== response.code) {
                  notification = new api.Notification(
                    response.code,
                    _.extend(notificationArgs, {
            ...
            request.done(function (response) {
              ...
              if ('publish' === response.changeset_status) {
                ...
                api.each(function (setting) {
                  ...
                  if (
                    setting._dirty &&
                    (_.isUndefined(api._latestSettingRevisions[setting.id]) ||
                      api._latestSettingRevisions[setting.id] <= latestRevision)
                  ) {
        ...
        trash: function trash() {
          ...
          api.notifications.add(
            new api.OverlayNotification('changeset_trashing', {
              type: 'info',
          ...
          fail = function (code, message) {
            ...
            api.notifications.add(
              new api.Notification(notificationCode, {
          ...
          request.fail(function (response) {
            ...
            if (
              response.success ||
              'non_existent_changeset' === code ||
              'changeset_already_trashed' === code
            ) {
              success(response.message);
            } else {
        ...
        getFrontendPreviewUrl: function () {
          ...
          if (
            api.state('changesetStatus').get() &&
            'publish' !== api.state('changesetStatus').get()
          ) {
    ...
    $([api.panel, api.section, api.control]).each(function (i, values) {
      var debouncedReflowPaneContents = _.debounce(
        api.reflowPaneContents,
    ...
    (function (state) {
      var saved = state.instance('saved'),
        ...
        defaultSelectedChangesetStatus;
      ...
      state.bind('change', function () {
        ...
        canSave =
          ...
          (!activated() ||
            !saved() ||
            (changesetStatus() !== selectedChangesetStatus() && '' !== changesetStatus()) ||
            ('future' === selectedChangesetStatus() &&
              changesetDate.get() !== selectedChangesetDate.get()));
      ...
      defaultSelectedChangesetStatus = api.settings.changeset.currentUserCanPublish
        ? 'publish'
        : 'draft';
      ...
      selectedChangesetStatus(
        '' === api.settings.changeset.status || 'auto-draft' === api.settings.changeset.status
          ? defaultSelectedChangesetStatus
          : api.settings.changeset.status
      );
      ...
      saved(true);
      if ('' === changesetStatus()) {
        // Handle case for loading starter content.
        api.each(function (setting) {
          if (setting._dirty) {
            saved(false);
          }
      ...
      if (api.settings.changeset.branching) {
        changesetStatus.bind(function (newStatus) {
          populateChangesetUuidParam(
            '' !== newStatus && 'publish' !== newStatus && 'trash' !== newStatus
          );
        });
      }
    })(api.state);
    ...
    (function checkAndDisplayLockNotice() {
          ...
          render: function () {
            ...
            api.requestChangesetUpdate({}, { autosave: true }).fail(function (response) {
              if (!response.autosaved) {
                li.find('.notice-error')
                  .prop('hidden', false)
                  .text(response.message || api.l10n.unknownRequestFail);
              }
            });
      ...
      function startLock(args) {
        ...
        api.notifications.add(
          new LockedNotification('changeset_locked', {
    ...
    (function () {
      ...
      function addSiteEditorNotification(notification) {
        api.notifications.add(
          new api.Notification('site_editor_block_theme_notice', {
      ...
      if (
        !api.settings.changeset.branching &&
        (!api.settings.changeset.status || 'auto-draft' === api.settings.changeset.status)
      ) {
      ...
      if (
        api.settings.changeset.latestAutoDraftUuid ||
        api.settings.changeset.hasAutosaveRevision
      ) {
        addAutosaveRestoreNotification();
      }
      var shouldDisplayBlockThemeNotification = !!parseInt(
        $('#customize-info').data('block-theme'),
        10
      );
    ...
    api.state('paneVisible').bind(function (paneVisible) {
      ...
      if (!paneVisible) {
        $('.collapse-sidebar').attr({
          'aria-expanded': 'false',
          'aria-label': api.l10n.expandSidebar,
        });
      } else {
        $('.collapse-sidebar').attr({
          'aria-expanded': 'true',
          'aria-label': api.l10n.collapseSidebar,
        });
      }
    });
    ...
    body.on('keydown', function (event) {
      ...
      if (
        event.target.closest('.block-editor-writing-flow') !== null ||
    ...
    (function initStickyHeaders() {
      ...
      changeContainer = function (container) {
        ...
        headerElement = newInstance.contentContainer
          .find('.customize-section-title, .panel-meta')
          .first();
      ...
      resetStickyHeader = function (headerElement, headerParent) {
        if (headerElement.hasClass('is-in-view')) {
          headerElement.removeClass('maybe-sticky is-in-view').css({
            width: '',
            top: '',
          });
      ...
      positionStickyHeader = function (header, scrollTop, scrollDirection) {
        ...
        if (!isScrollingUp) {
          if (isSticky) {
            headerTop = scrollTop;
            headerElement.removeClass('is-sticky').css({
              top: headerTop + 'px',
              width: '',
            });
          }
        ...
        if (!maybeSticky && scrollTop >= headerHeight) {
          ...
          headerElement.removeClass('maybe-sticky is-in-view is-sticky').css({
            top: '',
            width: '',
          });
          headerParent.css('padding-top', '');
        ...
        if (isInView && !isSticky) {
          ...
          if (headerTop >= scrollTop) {
            ...
            headerElement.addClass('is-sticky').css({
          ...
          headerElement.addClass('is-in-view').css('top', scrollTop - headerHeight + 'px');
    ...
    api.previewedDevice.bind(function (newDevice) {
      ...
      footerActions.find('.devices button').removeClass('active').attr('aria-pressed', false);

      footerActions
        .find('.devices .preview-' + newDevice)
        .addClass('active')
      ...
      overlay.removeClass(devices).addClass('preview-' + newDevice);
    });
    ...
    (function () {
      ...
      function requestClose() {
        ...
        if (dismissLock || dismissAutoSave) {
          wp.ajax
            .send('customize_dismiss_autosave_or_lock', {
    ...
    api(
      ...
      'page_on_front',
      'page_for_posts',
      function (showOnFront, pageOnFront, pageForPosts) {
        var handleChange = function () {
          var setting = this,
            pageOnFrontId,
            pageForPostsId,
            errorCode = 'show_on_front_page_collision';
          ...
          if (
            ...
            pageOnFrontId &&
            pageForPostsId &&
            pageOnFrontId === pageForPostsId
          ) {
            showOnFront.notifications.add(
              new api.Notification(errorCode, {
                type: 'error',
        ...
        api.control('show_on_front', function (showOnFrontControl) {
          showOnFrontControl.deferred.embedded.done(function () {
            showOnFrontControl.container.append(
              showOnFrontControl.getNotificationsContainerElement()
            );
          });
        });
      }
    ...
    (function () {
      ...
      sectionReady.done(function setupSectionDescription(section) {
        ...
        section.container
          .find('.section-description-buttons .section-description-close')
          .on('click', function () {
            section.container
              .find('.section-meta .customize-section-description:first')
              .removeClass('open')
              .slideUp();

            section.container.find('.customize-help-toggle').attr('aria-expanded', 'false').focus(); // Avoid focus loss.
          });
    ...
    api.control('header_video', function (headerVideoControl) {
      headerVideoControl.deferred.embedded.done(function () {
        var toggleNotice = function () {
          var section = api.section(headerVideoControl.section()),
            noticeCode = 'video_header_not_available';
          if (!section) {
            return;
          }
          if (headerVideoControl.active.get()) {
            ...
            section.notifications.add(
              new api.Notification(noticeCode, {
                type: 'info',
    ...
    api.previewer.bind(
      ...
      function handleSelectiveRefreshedSettingValidities(settingValidities) {
        api._handleSettingValidities({
    ...
    api.state('paneVisible').bind(function (isPaneVisible) {
      ...
      api
        .state('editShortcutVisibility')
        .set(isPaneVisible || isMobileScreen ? 'visible' : 'hidden');
    });
    if (window.matchMedia) {
    ...
    $(document).one('tinymce-editor-setup', function () {
      if (
        ...
        (!window.tinymce.ui.FloatPanel.zIndex || window.tinymce.ui.FloatPanel.zIndex < 500001)
      ) {
