...
(function ($) {
  ...
  api = window.wpNavMenu = {
    ...
    init: function () {
      ...
      if (api.menuList.length) this.initSortables();
      ...
      $.fn.extend({
        ...
        updateDepthClass: function (current, prev) {
          return this.each(function () {
            ...
            $(this)
              .removeClass('menu-item-depth-' + prev)
        ...
        moveHorizontally: function (newDepth, depth) {
          return this.each(function () {
            ...
            if (0 === newDepth) subItemText.hide();
            else subItemText.show();
          });
        },
        updateParentMenuItemDBId: function () {
          return this.each(function () {
            ...
            if (0 === depth) {
              // Item is on the top level, has no parent.
              input.val(0);
            } else {
          ...
          return this.each(function () {
            var t = $(this),
              ...
              checkboxes =
                ...
                0 === t.find('.tabs-panel-active .categorychecklist li input:checked').length
                  ? t.find('#page-all li input[type="checkbox"]')
                  : t.find('.tabs-panel-active .categorychecklist li input:checked'),
              re = /menu-item\[([^\]]*)/;
            ...
            $(checkboxes).each(function () {
              var t = $(this),
                ...
                listItemDBID =
                  'undefined' == typeof listItemDBIDMatch[1]
                    ? 0
        ...
        getItemData: function (itemType, id) {
          ...
          this.find('input').each(function () {
            ...
            i = fields.length;
            while (i--) {
              if (itemType == 'menu-item') field = fields[i] + '[' + id + ']';
        ...
        setItemData: function (itemData, itemType, id) {
          // Can take a type, such as 'menu-item', or an id.
          itemType = itemType || 'menu-item';
          ...
          this.find('input').each(function () {
            ...
            $.each(itemData, function (attr, val) {
              if (itemType == 'menu-item') field = attr + '[' + id + ']';
              else if (itemType == 'add-menu-item') field = 'menu-item[' + id + '][' + attr + ']';
          ...
          return this.each(function () {
            ...
            $.each(parentDropdowns, function () {
              var parentDropdown = $(this),
                $html = '',
                $selected = '',
                currentItemID = parentDropdown
                  .closest('li.menu-item')
                  .find('.menu-item-data-db-id')
                  .val(),
                currentparentID = parentDropdown
                  .closest('li.menu-item')
              ...
              $html +=
                ...
                $selected +
                ' value="0">' +
                wp.i18n._x('No Parent', 'menu item without a parent in navigation menu') +
                '</option>';
              ...
              $.each(menuItems, function () {
                ...
                if (!excludeMenuItem.includes(menuID)) {
                  ...
                  $html +=
                    '<option ' + $selected + ' value="' + menuID + '">' + menuTitle + '</option>';
                }
              });
            ...
            $.each(orderDropdowns, function () {
              ...
              if (isPrimaryMenuItem) {
                ...
                for (let i = 1; i < totalMenuItems + 1; i++) {
                  ...
                  $html +=
                    '<option ' + $selected + ' value="' + i + '">' + itemString + '</option>';
                }
              } else {
                var parentItem = menuItem
                    .prevAll('.menu-item-depth-' + parseInt(depth - 1, 10))
                    .first(),
                  parentItemId = parentItem.find('.menu-item-data-db-id').val(),
                  subItems = $(
                    '.menu-item .menu-item-data-parent-id[value="' + parentItemId + '"]'
                  ),
                  totalSubMenuItems = subItems.length;

                itemPosition =
                  $(subItems.parents('.menu-item').get().reverse()).index(menuItem) + 1;
                ...
                for (let i = 1; i < totalSubMenuItems + 1; i++) {
                  ...
                  $html +=
                    '<option ' + $selected + ' value="' + i + '">' + submenuString + '</option>';
                }
              }
      ...
      switch (dir) {
        case 'up':
          ...
          if (thisItemChildren) {
            ...
            thisItem
              .detach()
              .insertBefore(menuItems.eq(newItemPosition))
        ...
        case 'down':
          ...
          if (thisItemChildren) {
            ...
            items
              .detach()
              .insertAfter(menuItems.eq(thisItemPosition + items.length))
              .updateParentMenuItemDBId();
          } else {
            ...
            thisItem
              .detach()
              .insertAfter(menuItems.eq(thisItemPosition + 1))
              .updateParentMenuItemDBId();
          }
          break;
        case 'top':
        ...
        case 'right':
          ...
          if (thisItemData['menu-item-parent-id'] === prevItemId) break;
          thisItem.shiftHorizontally(1);
          break;
      }
      $this.trigger('focus');
    ...
    changeMenuParent: function (parentDropdown) {
      ...
      menuItem
        .insertAfter(menuItems.eq(menuItemNewPosition))
        .updateParentMenuItemDBId()
    ...
    refreshAdvancedAccessibilityOfItem: function (itemToRefresh) {
      ...
      var thisLink,
        ...
        prevItemNameLeft = menuItem
          .prevAll('.menu-item-depth-' + prevItemDepth)
          .first()
          .find('.menu-item-title')
          .text(),
        prevItemNameRight = menuItem
          .prevAll('.menu-item-depth-' + depth)
          .first()
          .find('.menu-item-title')
          .text(),
      ...
      if (!isPrimaryMenuItem) {
        ...
        thisLink
          .attr('aria-label', menus.moveOutFrom.replace('%s', prevItemNameLeft))
          .text(thisLinkText)
          .css('display', 'inline');
      }
      ...
      if (0 !== position) {
        if (
          menuItem.find('.menu-item-data-parent-id').val() !==
          menuItem.prev().find('.menu-item-data-db-id').val()
        ) {
          ((thisLink = menuItem.find('.menus-move-right')),
            (thisLinkText = menus.under.replace('%s', prevItemNameRight)));
          thisLink
            .attr('aria-label', menus.moveUnder.replace('%s', prevItemNameRight))
            .text(thisLinkText)
            .css('display', 'inline');
        }
      }
        ...
        .on('focus', function () {
          $(this)
            ...
            .on('keydown', function (e) {
              ...
              if (1 === $('#menu-to-edit li').length) return;
    ...
    initPreviewing: function () {
      ...
      $('#menu-to-edit').on('change input', '.edit-menu-item-title', function (e) {
        ...
        if (title) {
          ...
          titleEl
            .text(wp.i18n._x('(no label)', 'missing menu item navigation label'))
            .addClass('no-title');
        }
      });
    },
    ...
    initToggles: function () {
      ...
      $('.hide-postbox-tog').on('click', function () {
        var hidden = $('.accordion-container li.accordion-section')
          .filter(':hidden')
          .map(function () {
            return this.id;
          })
    ...
    initSortables: function () {
      ...
      if (0 !== $('#menu-to-edit li').length) $('.drag-instructions').show();
      ...
      api.menuList.sortable({
        ...
        start: function (e, ui) {
          ...
          if (api.isRTL) ui.item[0].style.right = 'auto';
        ...
        stop: function (e, ui) {
          ...
          if (0 < currentDepth) subMenuTitle.show();
          else subMenuTitle.hide();
      ...
      function updateMenuMaxDepth(depthChange) {
        ...
        if (depthChange === 0) {
          ...
          if (depth > menuMaxDepth) newDepth = depth;
    ...
    initManageLocations: function () {
      ...
      $('.menu-location-menus select').on('change', function () {
        ...
        if ($(this).find('option:selected').data('orig')) editLink.show();
        else editLink.hide();
      });
    },
    ...
    attachPendingMenuItemsListForDeletion: function () {
      $('#post-body-content').on('change', '.menu-item-checkbox', function () {
        ...
        listedMenuItem = $('#pending-menu-items-to-delete ul').find(
        ...
        if (this.checked === true) {
          $('#pending-menu-items-to-delete ul').append(
            '<li data-menu-item-id="' +
              menuItemID +
              '">' +
              '<span class="pending-menu-item-name">' +
              menuItemName +
              '</span> ' +
              '<span class="pending-menu-item-type">(' +
              menuItemType +
              ')</span>' +
    ...
    attachQuickSearchListeners: function () {
      ...
      $('#nav-menu-meta')
        .on('input', '.quick-search', function () {
          var $this = $(this);
    ...
    attachUnsavedChangesListener: function () {
      $(
        '#menu-management input, #menu-management select, #menu-management, #menu-management textarea, .menu-location-menus select'
      ).on('change', function () {
        api.registerChange();
      });
    ...
    attachTabsPanelListeners: function () {
      $('#menu-settings-column').on('click', function (e) {
        ...
        if (target.hasClass('nav-tab-link')) {
          ...
          if (selectAreaMatch) {
            items = $('#' + selectAreaMatch + ' .tabs-panel-active .menu-item-title input');
            selectAll = $(
              '.button-controls[data-items-type="' + selectAreaMatch + '"] .select-all'
            );
      ...
      $('#nav-menu-meta').on('click', 'a.page-numbers', function () {
        ...
        $.post(
          ajaxurl,
          this.href.replace(/.*\?/, '').replace(/action=([^&]*)/, '') + '&action=menu-get-metabox',
          function (resp) {
    ...
    eventOnClickCancelLink: function (clickedEl) {
      ...
      thisMenuItem
        .find('.menu-item-title')
        .text(settings.data('menu-item-data')['menu-item-title']);
    ...
    processQuickSearchQueryResponse: function (resp, req, panel) {
      ...
      if (!$items.length) {
        $('.categorychecklist', panel).html(
          '<li><p>' + wp.i18n.__('No results found.') + '</p></li>'
        );
  ...
  $(function () {
    ...
    $(
      '.menu-edit a, .menu-edit button, .menu-edit input, .menu-edit textarea, .menu-edit select'
    ).on('focus', function () {
